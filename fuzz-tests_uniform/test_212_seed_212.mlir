module {
  func.func @helper0(%arg0: i32, %arg1: !simt_step.resource<Global, i32>, %arg2: i32, %arg3: i32) {
    %c2_i32 = arith.constant 2 : i32
    %0 = arith.remsi %arg3, %c2_i32 : i32
    %1 = simt_step.lane_id
    %2 = arith.index_cast %1 : index to i32
    %3 = "simt_step.switch"(%0, %2) ({
    ^bb0(%arg4: i32):
      %c3_i32_2 = arith.constant 3 : i32
      %14 = arith.addi %arg0, %c3_i32_2 : i32
      "simt_step.yield"(%14) {fallthrough = false} : (i32) -> ()
    ^bb1(%15: i32):  // no predecessors
      %c0_i32_3 = arith.constant 0 : i32
      %c0_i32_4 = arith.constant 0 : i32
      %16:2 = "simt_step.loop"(%c0_i32_3, %c0_i32_4) ({
      ^bb0(%arg5: i32, %arg6: i32):
        %c4_i32_5 = arith.constant 4 : i32
        %17 = arith.remsi %arg3, %c4_i32_5 : i32
        %c1_i32 = arith.constant 1 : i32
        %18 = arith.addi %17, %c1_i32 : i32
        %19 = arith.cmpi slt, %arg6, %18 : i32
        "simt_step.condition"(%19, %arg5, %arg6) : (i1, i32, i32) -> ()
      }, {
      ^bb0(%arg5: i32, %arg6: i32):
        %c2_i32_5 = arith.constant 2 : i32
        %17 = arith.addi %arg0, %c2_i32_5 : i32
        %c1_i32 = arith.constant 1 : i32
        %18 = arith.addi %arg6, %c1_i32 : i32
        "simt_step.yield"(%17, %18) : (i32, i32) -> ()
      }) : (i32, i32) -> (i32, i32)
      %c4_i32 = arith.constant 4 : i32
      "simt_step.yield"(%16#0) {fallthrough = false} : (i32) -> ()
    }) {case_values = array<i64: 0>, default_index = 0 : i64} : (i32, i32) -> i32
    %true = arith.constant true
    %4 = "simt_step.wave_count_bits"(%true) : (i1) -> i32
    %c0_i32 = arith.constant 0 : i32
    %5 = arith.addi %c0_i32, %arg0 : i32
    "simt_step.buffer.store"(%arg1, %5, %4) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
    %c3_i32 = arith.constant 3 : i32
    %6 = arith.remsi %arg3, %c3_i32 : i32
    %7 = simt_step.lane_id
    %8 = arith.index_cast %7 : index to i32
    %9 = "simt_step.switch"(%6, %8) ({
    ^bb0(%arg4: i32):
      %c2_i32_2 = arith.constant 2 : i32
      %14 = arith.remsi %arg3, %c2_i32_2 : i32
      %c4_i32 = arith.constant 4 : i32
      %15 = arith.addi %arg0, %c4_i32 : i32
      %16 = "simt_step.switch"(%14, %15) ({
      ^bb0(%arg5: i32):
        %c2_i32_9 = arith.constant 2 : i32
        %29 = arith.remsi %arg3, %c2_i32_9 : i32
        %c2_i32_10 = arith.constant 2 : i32
        %30 = "simt_step.switch"(%29, %c2_i32_10) ({
        ^bb0(%arg6: i32):
          "simt_step.yield"(%arg0) {fallthrough = true} : (i32) -> ()
        ^bb1(%33: i32):  // no predecessors
          "simt_step.yield"(%arg0) {fallthrough = false} : (i32) -> ()
        }) {case_values = array<i64: 0>, default_index = 1 : i64} : (i32, i32) -> i32
        "simt_step.yield"(%30) {fallthrough = true} : (i32) -> ()
      ^bb1(%31: i32):  // no predecessors
        %c2_i32_11 = arith.constant 2 : i32
        %32 = arith.addi %arg0, %c2_i32_11 : i32
        "simt_step.yield"(%32) {fallthrough = false} : (i32) -> ()
      }) {case_values = array<i64: 0>, default_index = 0 : i64} : (i32, i32) -> i32
      %c4_i32_3 = arith.constant 4 : i32
      %17 = arith.addi %arg0, %c4_i32_3 : i32
      "simt_step.yield"(%16) {fallthrough = false} : (i32) -> ()
    ^bb1(%18: i32):  // no predecessors
      %c3_i32_4 = arith.constant 3 : i32
      %19 = arith.remsi %arg3, %c3_i32_4 : i32
      %20 = "simt_step.switch"(%19, %arg0) ({
      ^bb0(%arg5: i32):
        %c0_i32_9 = arith.constant 0 : i32
        %29 = arith.cmpi ne, %arg2, %c0_i32_9 : i32
        %30 = "simt_step.if"(%29) ({
          "simt_step.yield"(%arg0) : (i32) -> ()
        }, {
          "simt_step.yield"(%arg0) : (i32) -> ()
        }) : (i1) -> i32
        %c1_i32 = arith.constant 1 : i32
        "simt_step.yield"(%30) {fallthrough = true} : (i32) -> ()
      ^bb1(%31: i32):  // no predecessors
        %c0_i32_10 = arith.constant 0 : i32
        %32 = arith.cmpi ne, %arg2, %c0_i32_10 : i32
        %33 = "simt_step.if"(%32) ({
          %c3_i32_14 = arith.constant 3 : i32
          "simt_step.yield"(%c3_i32_14) : (i32) -> ()
        }, {
          %c0_i32_14 = arith.constant 0 : i32
          %37 = arith.addi %arg0, %c0_i32_14 : i32
          "simt_step.yield"(%37) : (i32) -> ()
        }) : (i1) -> i32
        "simt_step.yield"(%33) {fallthrough = true} : (i32) -> ()
      ^bb2(%34: i32):  // no predecessors
        %c0_i32_11 = arith.constant 0 : i32
        %c0_i32_12 = arith.constant 0 : i32
        %35:2 = "simt_step.loop"(%c0_i32_11, %c0_i32_12) ({
        ^bb0(%arg6: i32, %arg7: i32):
          %c4_i32_14 = arith.constant 4 : i32
          %37 = arith.remsi %arg3, %c4_i32_14 : i32
          %c1_i32_15 = arith.constant 1 : i32
          %38 = arith.addi %37, %c1_i32_15 : i32
          %39 = arith.cmpi slt, %arg7, %38 : i32
          "simt_step.condition"(%39, %arg6, %arg7) : (i1, i32, i32) -> ()
        }, {
        ^bb0(%arg6: i32, %arg7: i32):
          %c3_i32_14 = arith.constant 3 : i32
          %37 = arith.addi %arg0, %c3_i32_14 : i32
          %c1_i32_15 = arith.constant 1 : i32
          %38 = arith.addi %arg7, %c1_i32_15 : i32
          "simt_step.yield"(%37, %38) : (i32, i32) -> ()
        }) : (i32, i32) -> (i32, i32)
        %c1_i32_13 = arith.constant 1 : i32
        %36 = arith.addi %arg0, %c1_i32_13 : i32
        "simt_step.yield"(%35#0) {fallthrough = false} : (i32) -> ()
      }) {case_values = array<i64: 0, 1>, default_index = 1 : i64} : (i32, i32) -> i32
      %c0_i32_5 = arith.constant 0 : i32
      %21 = arith.addi %arg0, %c0_i32_5 : i32
      "simt_step.yield"(%20) {fallthrough = false} : (i32) -> ()
    ^bb2(%22: i32):  // no predecessors
      %c3_i32_6 = arith.constant 3 : i32
      %23 = arith.remsi %arg3, %c3_i32_6 : i32
      %24 = "simt_step.switch"(%23, %arg0) ({
      ^bb0(%arg5: i32):
        %c2_i32_9 = arith.constant 2 : i32
        %29 = arith.remsi %arg3, %c2_i32_9 : i32
        %30 = "simt_step.switch"(%29, %arg0) ({
        ^bb0(%arg6: i32):
          %c3_i32_16 = arith.constant 3 : i32
          "simt_step.yield"(%c3_i32_16) {fallthrough = false} : (i32) -> ()
        ^bb1(%39: i32):  // no predecessors
          %c3_i32_17 = arith.constant 3 : i32
          "simt_step.yield"(%c3_i32_17) {fallthrough = false} : (i32) -> ()
        }) {case_values = array<i64: 0>, default_index = 1 : i64} : (i32, i32) -> i32
        %c3_i32_10 = arith.constant 3 : i32
        %31 = arith.addi %arg0, %c3_i32_10 : i32
        "simt_step.yield"(%30) {fallthrough = true} : (i32) -> ()
      ^bb1(%32: i32):  // no predecessors
        %c0_i32_11 = arith.constant 0 : i32
        %c0_i32_12 = arith.constant 0 : i32
        %33:2 = "simt_step.loop"(%c0_i32_11, %c0_i32_12) ({
        ^bb0(%arg6: i32, %arg7: i32):
          %c4_i32_16 = arith.constant 4 : i32
          %39 = arith.remsi %arg3, %c4_i32_16 : i32
          %c1_i32 = arith.constant 1 : i32
          %40 = arith.addi %39, %c1_i32 : i32
          %41 = arith.cmpi slt, %arg7, %40 : i32
          "simt_step.condition"(%41, %arg6, %arg7) : (i1, i32, i32) -> ()
        }, {
        ^bb0(%arg6: i32, %arg7: i32):
          %c1_i32 = arith.constant 1 : i32
          %39 = arith.addi %arg7, %c1_i32 : i32
          "simt_step.break"(%arg0, %39) : (i32, i32) -> ()
        }) : (i32, i32) -> (i32, i32)
        %c0_i32_13 = arith.constant 0 : i32
        %34 = arith.addi %arg0, %c0_i32_13 : i32
        "simt_step.yield"(%33#0) {fallthrough = true} : (i32) -> ()
      ^bb2(%35: i32):  // no predecessors
        "simt_step.yield"(%arg0) {fallthrough = false} : (i32) -> ()
      ^bb3(%36: i32):  // no predecessors
        %c0_i32_14 = arith.constant 0 : i32
        %37 = arith.cmpi ne, %arg2, %c0_i32_14 : i32
        %38 = "simt_step.if"(%37) ({
          "simt_step.yield"(%arg0) : (i32) -> ()
        }, {
          %c2_i32_16 = arith.constant 2 : i32
          %39 = arith.addi %arg0, %c2_i32_16 : i32
          "simt_step.yield"(%39) : (i32) -> ()
        }) : (i1) -> i32
        %c4_i32_15 = arith.constant 4 : i32
        "simt_step.yield"(%38) {fallthrough = false} : (i32) -> ()
      }) {case_values = array<i64: 0, 1, 2>, default_index = 3 : i64} : (i32, i32) -> i32
      %c0_i32_7 = arith.constant 0 : i32
      %25 = arith.addi %arg0, %c0_i32_7 : i32
      "simt_step.yield"(%24) {fallthrough = false} : (i32) -> ()
    ^bb3(%26: i32):  // no predecessors
      %c3_i32_8 = arith.constant 3 : i32
      %27 = arith.remsi %arg3, %c3_i32_8 : i32
      %28 = "simt_step.switch"(%27, %arg0) ({
      ^bb0(%arg5: i32):
        %c0_i32_9 = arith.constant 0 : i32
        %c0_i32_10 = arith.constant 0 : i32
        %29:2 = "simt_step.loop"(%c0_i32_9, %c0_i32_10) ({
        ^bb0(%arg6: i32, %arg7: i32):
          %c4_i32_16 = arith.constant 4 : i32
          %37 = arith.remsi %arg3, %c4_i32_16 : i32
          %c1_i32 = arith.constant 1 : i32
          %38 = arith.addi %37, %c1_i32 : i32
          %39 = arith.cmpi slt, %arg7, %38 : i32
          "simt_step.condition"(%39, %arg6, %arg7) : (i1, i32, i32) -> ()
        }, {
        ^bb0(%arg6: i32, %arg7: i32):
          %c2_i32_16 = arith.constant 2 : i32
          %37 = arith.addi %arg0, %c2_i32_16 : i32
          %c1_i32 = arith.constant 1 : i32
          %38 = arith.addi %arg7, %c1_i32 : i32
          "simt_step.yield"(%37, %38) : (i32, i32) -> ()
        }) : (i32, i32) -> (i32, i32)
        %c2_i32_11 = arith.constant 2 : i32
        %30 = arith.addi %arg0, %c2_i32_11 : i32
        "simt_step.yield"(%29#0) {fallthrough = false} : (i32) -> ()
      ^bb1(%31: i32):  // no predecessors
        %c0_i32_12 = arith.constant 0 : i32
        %32 = arith.cmpi ne, %arg2, %c0_i32_12 : i32
        %33 = "simt_step.if"(%32) ({
          "simt_step.yield"(%arg0) : (i32) -> ()
        }, {
          %c3_i32_16 = arith.constant 3 : i32
          "simt_step.yield"(%c3_i32_16) : (i32) -> ()
        }) : (i1) -> i32
        "simt_step.yield"(%33) {fallthrough = false} : (i32) -> ()
      ^bb2(%34: i32):  // no predecessors
        %c0_i32_13 = arith.constant 0 : i32
        %c0_i32_14 = arith.constant 0 : i32
        %35:2 = "simt_step.loop"(%c0_i32_13, %c0_i32_14) ({
        ^bb0(%arg6: i32, %arg7: i32):
          %c4_i32_16 = arith.constant 4 : i32
          %37 = arith.remsi %arg3, %c4_i32_16 : i32
          %c1_i32 = arith.constant 1 : i32
          %38 = arith.addi %37, %c1_i32 : i32
          %39 = arith.cmpi slt, %arg7, %38 : i32
          "simt_step.condition"(%39, %arg6, %arg7) : (i1, i32, i32) -> ()
        }, {
        ^bb0(%arg6: i32, %arg7: i32):
          %c1_i32 = arith.constant 1 : i32
          %37 = arith.addi %arg7, %c1_i32 : i32
          "simt_step.yield"(%arg0, %37) : (i32, i32) -> ()
        }) : (i32, i32) -> (i32, i32)
        %c2_i32_15 = arith.constant 2 : i32
        %36 = arith.addi %arg0, %c2_i32_15 : i32
        "simt_step.yield"(%35#0) {fallthrough = false} : (i32) -> ()
      }) {case_values = array<i64: 0, 1>, default_index = 1 : i64} : (i32, i32) -> i32
      "simt_step.yield"(%28) {fallthrough = false} : (i32) -> ()
    }) {case_values = array<i64: 0, 1, 2>, default_index = 2 : i64} : (i32, i32) -> i32
    %true_0 = arith.constant true
    %10 = "simt_step.wave_count_bits"(%true_0) : (i1) -> i32
    %c16_i32 = arith.constant 16 : i32
    %11 = arith.addi %c16_i32, %arg0 : i32
    "simt_step.buffer.store"(%arg1, %11, %10) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
    %c0_i32_1 = arith.constant 0 : i32
    %12 = arith.cmpi ne, %arg2, %c0_i32_1 : i32
    %13 = arith.select %12, %3, %9 : i32
    return
  }
  func.func @main(%arg0: !simt_step.resource<Global, i32>, %arg1: !simt_step.resource<Global, i32>) attributes {simt.num_threads = [4, 1, 1], simt.subgroup_width = 8 : i64} {
    %0 = "simt_step.dispatch_thread_id"() : () -> i32
    %c0_i32 = arith.constant 0 : i32
    %1 = arith.addi %c0_i32, %0 : i32
    %2 = "simt_step.buffer.load"(%arg1, %1) : (!simt_step.resource<Global, i32>, i32) -> i32
    %c4_i32 = arith.constant 4 : i32
    %3 = arith.addi %c4_i32, %0 : i32
    %4 = "simt_step.buffer.load"(%arg1, %3) : (!simt_step.resource<Global, i32>, i32) -> i32
    call @helper0(%0, %arg0, %2, %4) : (i32, !simt_step.resource<Global, i32>, i32, i32) -> ()
    %c8_i32 = arith.constant 8 : i32
    %5 = arith.addi %c8_i32, %0 : i32
    %6 = "simt_step.buffer.load"(%arg1, %5) : (!simt_step.resource<Global, i32>, i32) -> i32
    %c0_i32_0 = arith.constant 0 : i32
    %7 = arith.cmpi ne, %6, %c0_i32_0 : i32
    %8 = "simt_step.if"(%7) ({
      %c12_i32 = arith.constant 12 : i32
      %11 = arith.addi %c12_i32, %0 : i32
      %12 = "simt_step.buffer.load"(%arg1, %11) : (!simt_step.resource<Global, i32>, i32) -> i32
      %c0_i32_1 = arith.constant 0 : i32
      %13 = arith.cmpi ne, %12, %c0_i32_1 : i32
      %14 = "simt_step.if"(%13) ({
        %c16_i32 = arith.constant 16 : i32
        %15 = arith.addi %c16_i32, %0 : i32
        %16 = "simt_step.buffer.load"(%arg1, %15) : (!simt_step.resource<Global, i32>, i32) -> i32
        %c0_i32_2 = arith.constant 0 : i32
        %17 = arith.cmpi ne, %16, %c0_i32_2 : i32
        %18 = "simt_step.if"(%17) ({
          "simt_step.yield"(%0) : (i32) -> ()
        }, {
          %19 = simt_step.lane_id
          %20 = arith.index_cast %19 : index to i32
          "simt_step.yield"(%20) : (i32) -> ()
        }) : (i1) -> i32
        "simt_step.yield"(%18) : (i32) -> ()
      }, {
        "simt_step.yield"(%0) : (i32) -> ()
      }) : (i1) -> i32
      "simt_step.yield"(%14) : (i32) -> ()
    }, {
      %c0_i32_1 = arith.constant 0 : i32
      %c0_i32_2 = arith.constant 0 : i32
      %11:2 = "simt_step.loop"(%c0_i32_1, %c0_i32_2) ({
      ^bb0(%arg2: i32, %arg3: i32):
        %c4_i32_4 = arith.constant 4 : i32
        %12 = arith.muli %arg3, %c4_i32_4 : i32
        %13 = arith.addi %12, %0 : i32
        %c20_i32 = arith.constant 20 : i32
        %14 = arith.addi %c20_i32, %13 : i32
        %15 = "simt_step.buffer.load"(%arg1, %14) : (!simt_step.resource<Global, i32>, i32) -> i32
        %c0_i32_5 = arith.constant 0 : i32
        %16 = arith.cmpi ne, %15, %c0_i32_5 : i32
        "simt_step.condition"(%16, %arg2, %arg3) : (i1, i32, i32) -> ()
      }, {
      ^bb0(%arg2: i32, %arg3: i32):
        %c40_i32 = arith.constant 40 : i32
        %12 = arith.addi %c40_i32, %0 : i32
        %13 = "simt_step.buffer.load"(%arg1, %12) : (!simt_step.resource<Global, i32>, i32) -> i32
        %14 = simt_step.lane_id
        %15 = arith.index_cast %14 : index to i32
        %16 = "simt_step.switch"(%13, %15) ({
        ^bb0(%arg4: i32):
          %c4_i32_5 = arith.constant 4 : i32
          %19 = arith.addi %0, %c4_i32_5 : i32
          "simt_step.yield"(%19) {fallthrough = false} : (i32) -> ()
        ^bb1(%20: i32):  // no predecessors
          %c3_i32 = arith.constant 3 : i32
          %21 = arith.addi %0, %c3_i32 : i32
          "simt_step.yield"(%21) {fallthrough = false} : (i32) -> ()
        }) {case_values = array<i64: 0>, default_index = 1 : i64} : (i32, i32) -> i32
        %true_4 = arith.constant true
        %17 = arith.addi %arg2, %16 : i32
        %c1_i32 = arith.constant 1 : i32
        %18 = arith.addi %arg3, %c1_i32 : i32
        "simt_step.yield"(%17, %18) : (i32, i32) -> ()
      }) : (i32, i32) -> (i32, i32)
      %true_3 = arith.constant true
      "simt_step.yield"(%11#0) : (i32) -> ()
    }) : (i1) -> i32
    %c32_i32 = arith.constant 32 : i32
    %9 = arith.addi %c32_i32, %0 : i32
    %true = arith.constant true
    %10 = "simt_step.wave_count_bits"(%true) : (i1) -> i32
    "simt_step.buffer.store"(%arg0, %9, %10) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
    return
  }
}

