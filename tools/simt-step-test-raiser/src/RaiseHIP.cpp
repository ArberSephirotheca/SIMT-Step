...
class HipRaiser : public BaseRaiser {
...
LogicalResult emitHarness(
    ...
    for (auto [i, abuf] : llvm::enumerate(props.input)){
        ...
        os << "};\n";
        os << "int *dev_actual" << i << ";\n";
        os << "hipMalloc(&dev_actual" << i << ", sizeof(host_actual" << i << "));\n";
        os << "hipMemcpy(dev_actual" << i << ", host_actual" << i << ", sizeof(host_actual" << i << "), hipMemcpyHostToDevice);\n";
    }
    ...
    for (size_t i = 0; i < props.input.size(); i++){
        os << "hipMemcpy(host_actual" << i << ", dev_actual" << i << ", sizeof(host_actual" << i << "), hipMemcpyDeviceToHost);\n";
...
LogicalResult printOp(GroupIdOp& op) override {
    return emitConstVec(op.getResult(), "make_int3(blockDim.x, blockDim.y, blockDim.z)");
}

LogicalResult printOp(GroupThreadIdOp& op) override {
    return emitConstVec(op.getResult(), "make_int3(threadIdx.x, threadIdx.y, threadIdx.z)");
}
