...
struct MslEmitter {
    ...
    std::string emitType(Type ty) const {
        if (auto resTy = mlir::dyn_cast<simt::dialect::ResourceType>(ty)) {
            return "device " + emitType(resTy.getElementType()) + "*";
        }
    ...
    LogicalResult emitSwitch(simt::dialect::SwitchOp sw) {
        ...
        if (caseValues.size() + 1 != numBlocks)
            llvm::report_fatal_error("MslEmitter: switch case_values size mismatch");
        auto defaultIndexAttr = sw.getDefaultIndexAttr();
        if (!defaultIndexAttr)
            llvm::report_fatal_error("MslEmitter: switch missing default_index");
        int64_t defaultIndex = defaultIndexAttr.getInt();
        if (defaultIndex < 0 ||
            static_cast<std::size_t>(defaultIndex) >= numBlocks)
            llvm::report_fatal_error("MslEmitter: switch default_index out of range");

        std::vector<bool> caseFallthrough;
        caseFallthrough.reserve(numBlocks);
        for (auto &blk : region) {
            if (blk.empty())
                llvm::report_fatal_error("MslEmitter: switch case missing yield");
            auto y = dyn_cast<simt::dialect::YieldOp>(blk.back());
            if (!y)
                llvm::report_fatal_error("MslEmitter: switch case missing yield");
            auto attr = y->getAttrOfType<mlir::BoolAttr>("fallthrough");
            if (!attr)
                llvm::report_fatal_error("MslEmitter: switch missing fallthrough attr");
        ...
        if (caseValueIndex != caseValues.size())
            llvm::report_fatal_error("MslEmitter: switch case_values mapping mismatch");
    ...
    LogicalResult emitOp(Operation *op) {
        ...
        if (auto call = dyn_cast<func::CallOp>(op)) {
            if (call.getNumResults() > 1)
                llvm::report_fatal_error("MslEmitter: call with multiple results");
        ...
        if (auto ret = dyn_cast<func::ReturnOp>(op)) {
            ...
            llvm::report_fatal_error("MslEmitter: multiple return values");
        }
...
static LogicalResult emitHelperFunction(func::FuncOp func,
    ...
    if (results.size() > 1)
        llvm::report_fatal_error("MslEmitter: helper multiple results");
...
LogicalResult emitModuleAsMsl(ModuleOp module, llvm::raw_ostream &os) {
    func::FuncOp func;
    ...
    if (subgroupWidth != 32) {
        os << "unsupported subgroup width for MSL: " << subgroupWidth << "\n";
        return failure();
    }
    ...
    for (auto arg : func.getArguments()) {
        ...
        first = false;
        if (mlir::isa<simt::dialect::ResourceType>(arg.getType())) {
            os << MslEmitter(os).emitType(arg.getType()) << " buf" << arg.getArgNumber()
               << " [[buffer(" << arg.getArgNumber() << ")]]";
        } else {
            os << MslEmitter(os).emitType(arg.getType()) << " arg" << arg.getArgNumber();
        }
    }
    if (!func.getArguments().empty())
        os << ", ";
    os << "uint3 tid3 [[thread_position_in_grid]]) {\n";
    os << "  int tid = static_cast<int>(tid3.x);\n";
