module {
  func.func @helper0(%arg0: i32, %arg1: !simt_step.resource<Global, i32>, %arg2: i32, %arg3: i32) {
    %true = arith.constant true
    %0 = "simt_step.wave_count_bits"(%true) : (i1) -> i32
    %c0_i32 = arith.constant 0 : i32
    %1 = arith.addi %c0_i32, %arg0 : i32
    "simt_step.buffer.store"(%arg1, %1, %0) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
    %c0_i32_0 = arith.constant 0 : i32
    %2 = arith.cmpi ne, %arg2, %c0_i32_0 : i32
    %3 = "simt_step.if"(%2) ({
      %true_8 = arith.constant true
      %12 = "simt_step.wave_count_bits"(%true_8) : (i1) -> i32
      %c16_i32 = arith.constant 16 : i32
      %13 = arith.addi %c16_i32, %arg0 : i32
      "simt_step.buffer.store"(%arg1, %13, %12) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
      "simt_step.yield"(%12) : (i32) -> ()
    }, {
      %true_8 = arith.constant true
      %12 = "simt_step.wave_count_bits"(%true_8) : (i1) -> i32
      %c32_i32 = arith.constant 32 : i32
      %13 = arith.addi %c32_i32, %arg0 : i32
      "simt_step.buffer.store"(%arg1, %13, %12) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
      "simt_step.yield"(%12) : (i32) -> ()
    }) : (i1) -> i32
    %c0_i32_1 = arith.constant 0 : i32
    %4 = arith.cmpi ne, %arg2, %c0_i32_1 : i32
    %5 = arith.select %4, %0, %3 : i32
    %c0_i32_2 = arith.constant 0 : i32
    %c0_i32_3 = arith.constant 0 : i32
    %6:2 = "simt_step.loop"(%c0_i32_2, %c0_i32_3) ({
    ^bb0(%arg4: i32, %arg5: i32):
      %c4_i32 = arith.constant 4 : i32
      %12 = arith.remsi %arg3, %c4_i32 : i32
      %c1_i32 = arith.constant 1 : i32
      %13 = arith.addi %12, %c1_i32 : i32
      %14 = arith.cmpi slt, %arg5, %13 : i32
      "simt_step.condition"(%14, %arg4, %arg5) : (i1, i32, i32) -> ()
    }, {
    ^bb0(%arg4: i32, %arg5: i32):
      %true_8 = arith.constant true
      %12 = "simt_step.wave_count_bits"(%true_8) : (i1) -> i32
      %c48_i32 = arith.constant 48 : i32
      %c4_i32 = arith.constant 4 : i32
      %13 = arith.muli %arg5, %c4_i32 : i32
      %14 = arith.addi %c48_i32, %13 : i32
      %15 = arith.addi %14, %arg0 : i32
      "simt_step.buffer.store"(%arg1, %15, %12) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
      %c1_i32 = arith.constant 1 : i32
      %16 = arith.addi %arg5, %c1_i32 : i32
      "simt_step.continue"(%12, %16) : (i32, i32) -> ()
    }) : (i32, i32) -> (i32, i32)
    %c0_i32_4 = arith.constant 0 : i32
    %7 = arith.cmpi ne, %arg2, %c0_i32_4 : i32
    %8 = arith.select %7, %5, %6#0 : i32
    %c0_i32_5 = arith.constant 0 : i32
    %c0_i32_6 = arith.constant 0 : i32
    %9:2 = "simt_step.loop"(%c0_i32_5, %c0_i32_6) ({
    ^bb0(%arg4: i32, %arg5: i32):
      %c4_i32 = arith.constant 4 : i32
      %12 = arith.remsi %arg3, %c4_i32 : i32
      %c1_i32 = arith.constant 1 : i32
      %13 = arith.addi %12, %c1_i32 : i32
      %14 = arith.cmpi slt, %arg5, %13 : i32
      "simt_step.condition"(%14, %arg4, %arg5) : (i1, i32, i32) -> ()
    }, {
    ^bb0(%arg4: i32, %arg5: i32):
      %c2_i32 = arith.constant 2 : i32
      %12 = arith.remsi %arg3, %c2_i32 : i32
      %13 = "simt_step.switch"(%12, %arg0) ({
      ^bb0(%arg6: i32):
        %c0_i32_9 = arith.constant 0 : i32
        %c0_i32_10 = arith.constant 0 : i32
        %19:2 = "simt_step.loop"(%c0_i32_9, %c0_i32_10) ({
        ^bb0(%arg7: i32, %arg8: i32):
          %c4_i32_13 = arith.constant 4 : i32
          %25 = arith.remsi %arg3, %c4_i32_13 : i32
          %c1_i32_14 = arith.constant 1 : i32
          %26 = arith.addi %25, %c1_i32_14 : i32
          %27 = arith.cmpi slt, %arg8, %26 : i32
          "simt_step.condition"(%27, %arg7, %arg8) : (i1, i32, i32) -> ()
        }, {
        ^bb0(%arg7: i32, %arg8: i32):
          %true_13 = arith.constant true
          %25 = "simt_step.wave_count_bits"(%true_13) : (i1) -> i32
          %c64_i32 = arith.constant 64 : i32
          %c4_i32_14 = arith.constant 4 : i32
          %26 = arith.muli %arg8, %c4_i32_14 : i32
          %27 = arith.addi %c64_i32, %26 : i32
          %28 = arith.addi %27, %arg0 : i32
          "simt_step.buffer.store"(%arg1, %28, %25) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
          %c1_i32_15 = arith.constant 1 : i32
          %29 = arith.addi %arg8, %c1_i32_15 : i32
          "simt_step.yield"(%25, %29) : (i32, i32) -> ()
        }) : (i32, i32) -> (i32, i32)
        "simt_step.yield"(%19#0) {fallthrough = true} : (i32) -> ()
      ^bb1(%20: i32):  // no predecessors
        %true_11 = arith.constant true
        %21 = "simt_step.wave_count_bits"(%true_11) : (i1) -> i32
        %c80_i32 = arith.constant 80 : i32
        %c4_i32_12 = arith.constant 4 : i32
        %22 = arith.muli %arg5, %c4_i32_12 : i32
        %23 = arith.addi %c80_i32, %22 : i32
        %24 = arith.addi %23, %arg0 : i32
        "simt_step.buffer.store"(%arg1, %24, %21) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
        "simt_step.yield"(%21) {fallthrough = false} : (i32) -> ()
      }) {case_values = array<i64: 0>, default_index = 0 : i64} : (i32, i32) -> i32
      %true_8 = arith.constant true
      %14 = "simt_step.wave_count_bits"(%true_8) : (i1) -> i32
      %c96_i32 = arith.constant 96 : i32
      %c4_i32 = arith.constant 4 : i32
      %15 = arith.muli %arg5, %c4_i32 : i32
      %16 = arith.addi %c96_i32, %15 : i32
      %17 = arith.addi %16, %arg0 : i32
      "simt_step.buffer.store"(%arg1, %17, %14) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
      %c1_i32 = arith.constant 1 : i32
      %18 = arith.addi %arg5, %c1_i32 : i32
      "simt_step.yield"(%13, %18) : (i32, i32) -> ()
    }) : (i32, i32) -> (i32, i32)
    %c0_i32_7 = arith.constant 0 : i32
    %10 = arith.cmpi ne, %arg2, %c0_i32_7 : i32
    %11 = arith.select %10, %8, %9#0 : i32
    return
  }
  func.func @main(%arg0: !simt_step.resource<Global, i32>, %arg1: !simt_step.resource<Global, i32>) attributes {simt.num_threads = [4, 1, 1], simt.subgroup_width = 8 : i64} {
    %0 = "simt_step.dispatch_thread_id"() : () -> i32
    %c0_i32 = arith.constant 0 : i32
    %1 = arith.addi %c0_i32, %0 : i32
    %2 = "simt_step.buffer.load"(%arg1, %1) : (!simt_step.resource<Global, i32>, i32) -> i32
    %c4_i32 = arith.constant 4 : i32
    %3 = arith.addi %c4_i32, %0 : i32
    %4 = "simt_step.buffer.load"(%arg1, %3) : (!simt_step.resource<Global, i32>, i32) -> i32
    %c0_i32_0 = arith.constant 0 : i32
    %5 = arith.cmpi ne, %2, %c0_i32_0 : i32
    "simt_step.if"(%5) ({
      "simt_step.yield"() : () -> ()
    }, {
      %c8_i32 = arith.constant 8 : i32
      %23 = arith.addi %c8_i32, %0 : i32
      %24 = "simt_step.buffer.load"(%arg1, %23) : (!simt_step.resource<Global, i32>, i32) -> i32
      %c0_i32_5 = arith.constant 0 : i32
      %25 = arith.cmpi ne, %24, %c0_i32_5 : i32
      "simt_step.if"(%25) ({
        "simt_step.yield"() : () -> ()
      }, {
        %c4_i32_6 = arith.constant 4 : i32
        %26 = arith.remsi %0, %c4_i32_6 : i32
        %27 = simt_step.subgroup_id
        %28 = arith.index_cast %27 : index to i32
        %29 = "simt_step.switch"(%26, %28) ({
        ^bb0(%arg2: i32):
          %c3_i32 = arith.constant 3 : i32
          %30 = arith.addi %arg2, %c3_i32 : i32
          "simt_step.yield"(%30) {fallthrough = false} : (i32) -> ()
        ^bb1(%31: i32):  // no predecessors
          %c2_i32 = arith.constant 2 : i32
          %32 = arith.addi %31, %c2_i32 : i32
          "simt_step.yield"(%32) {fallthrough = false} : (i32) -> ()
        ^bb2(%33: i32):  // no predecessors
          %c1_i32 = arith.constant 1 : i32
          %34 = arith.addi %33, %c1_i32 : i32
          "simt_step.yield"(%34) {fallthrough = false} : (i32) -> ()
        ^bb3(%35: i32):  // no predecessors
          %c2_i32_7 = arith.constant 2 : i32
          %36 = arith.addi %35, %c2_i32_7 : i32
          "simt_step.yield"(%36) {fallthrough = false} : (i32) -> ()
        }) {case_values = array<i64: 0, 1, 2>, default_index = 0 : i64} : (i32, i32) -> i32
        func.call @helper0(%0, %arg0, %2, %4) : (i32, !simt_step.resource<Global, i32>, i32, i32) -> ()
        "simt_step.yield"() : () -> ()
      }) : (i1) -> ()
      "simt_step.yield"() : () -> ()
    }) : (i1) -> ()
    %c12_i32 = arith.constant 12 : i32
    %6 = arith.addi %c12_i32, %0 : i32
    %7 = "simt_step.buffer.load"(%arg1, %6) : (!simt_step.resource<Global, i32>, i32) -> i32
    %c0_i32_1 = arith.constant 0 : i32
    %8 = arith.cmpi ne, %7, %c0_i32_1 : i32
    %9 = "simt_step.if"(%8) ({
      "simt_step.yield"(%0) : (i32) -> ()
    }, {
      %c16_i32 = arith.constant 16 : i32
      %23 = arith.addi %c16_i32, %0 : i32
      %24 = "simt_step.buffer.load"(%arg1, %23) : (!simt_step.resource<Global, i32>, i32) -> i32
      %25 = simt_step.subgroup_id
      %26 = arith.index_cast %25 : index to i32
      %27 = "simt_step.switch"(%24, %26) ({
      ^bb0(%arg2: i32):
        "simt_step.yield"(%0) {fallthrough = true} : (i32) -> ()
      ^bb1(%28: i32):  // no predecessors
        %29 = simt_step.lane_id
        %30 = arith.index_cast %29 : index to i32
        %true_5 = arith.constant true
        %c112_i32 = arith.constant 112 : i32
        %31 = arith.addi %c112_i32, %0 : i32
        %true_6 = arith.constant true
        %32 = "simt_step.wave_count_bits"(%true_6) : (i1) -> i32
        "simt_step.buffer.store"(%arg0, %31, %32) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
        "simt_step.yield"(%30) {fallthrough = false} : (i32) -> ()
      }) {case_values = array<i64: 0>, default_index = 1 : i64} : (i32, i32) -> i32
      "simt_step.yield"(%27) : (i32) -> ()
    }) : (i1) -> i32
    %c128_i32 = arith.constant 128 : i32
    %10 = arith.addi %c128_i32, %0 : i32
    %true = arith.constant true
    %11 = "simt_step.wave_count_bits"(%true) : (i1) -> i32
    "simt_step.buffer.store"(%arg0, %10, %11) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
    %c20_i32 = arith.constant 20 : i32
    %12 = arith.addi %c20_i32, %0 : i32
    %13 = "simt_step.buffer.load"(%arg1, %12) : (!simt_step.resource<Global, i32>, i32) -> i32
    %14 = simt_step.subgroup_id
    %15 = arith.index_cast %14 : index to i32
    %16 = "simt_step.switch"(%13, %15) ({
    ^bb0(%arg2: i32):
      %c24_i32 = arith.constant 24 : i32
      %23 = arith.addi %c24_i32, %0 : i32
      %24 = "simt_step.buffer.load"(%arg1, %23) : (!simt_step.resource<Global, i32>, i32) -> i32
      %c4_i32_5 = arith.constant 4 : i32
      %25 = "simt_step.switch"(%24, %c4_i32_5) ({
      ^bb0(%arg3: i32):
        %c28_i32 = arith.constant 28 : i32
        %54 = arith.addi %c28_i32, %0 : i32
        %55 = "simt_step.buffer.load"(%arg1, %54) : (!simt_step.resource<Global, i32>, i32) -> i32
        %56 = simt_step.subgroup_id
        %57 = arith.index_cast %56 : index to i32
        %58 = "simt_step.switch"(%55, %57) ({
        ^bb0(%arg4: i32):
          %c4_i32_28 = arith.constant 4 : i32
          %75 = arith.addi %0, %c4_i32_28 : i32
          %true_29 = arith.constant true
          %c144_i32 = arith.constant 144 : i32
          %76 = arith.addi %c144_i32, %0 : i32
          %true_30 = arith.constant true
          %77 = "simt_step.wave_count_bits"(%true_30) : (i1) -> i32
          "simt_step.buffer.store"(%arg0, %76, %77) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
          "simt_step.yield"(%75) {fallthrough = false} : (i32) -> ()
        ^bb1(%78: i32):  // no predecessors
          %c0_i32_31 = arith.constant 0 : i32
          %79 = arith.addi %0, %c0_i32_31 : i32
          "simt_step.yield"(%79) {fallthrough = false} : (i32) -> ()
        }) {case_values = array<i64: 0>, default_index = 1 : i64} : (i32, i32) -> i32
        "simt_step.yield"(%58) {fallthrough = false} : (i32) -> ()
      ^bb1(%59: i32):  // no predecessors
        %c0_i32_18 = arith.constant 0 : i32
        %c0_i32_19 = arith.constant 0 : i32
        %60:2 = "simt_step.loop"(%c0_i32_18, %c0_i32_19) ({
        ^bb0(%arg4: i32, %arg5: i32):
          %c4_i32_28 = arith.constant 4 : i32
          %75 = arith.muli %arg5, %c4_i32_28 : i32
          %76 = arith.addi %75, %0 : i32
          %c32_i32 = arith.constant 32 : i32
          %77 = arith.addi %c32_i32, %76 : i32
          %78 = "simt_step.buffer.load"(%arg1, %77) : (!simt_step.resource<Global, i32>, i32) -> i32
          %c0_i32_29 = arith.constant 0 : i32
          %79 = arith.cmpi ne, %78, %c0_i32_29 : i32
          "simt_step.condition"(%79, %arg4, %arg5) : (i1, i32, i32) -> ()
        }, {
        ^bb0(%arg4: i32, %arg5: i32):
          %75 = arith.addi %arg4, %arg5 : i32
          %c1_i32 = arith.constant 1 : i32
          %76 = arith.addi %arg5, %c1_i32 : i32
          %true_28 = arith.constant true
          %c160_i32 = arith.constant 160 : i32
          %c4_i32_29 = arith.constant 4 : i32
          %77 = arith.muli %arg5, %c4_i32_29 : i32
          %78 = arith.addi %c160_i32, %77 : i32
          %79 = arith.addi %78, %0 : i32
          %true_30 = arith.constant true
          %80 = "simt_step.wave_count_bits"(%true_30) : (i1) -> i32
          "simt_step.buffer.store"(%arg0, %79, %80) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
          "simt_step.yield"(%75, %76) : (i32, i32) -> ()
        }) : (i32, i32) -> (i32, i32)
        %true_20 = arith.constant true
        %c176_i32 = arith.constant 176 : i32
        %61 = arith.addi %c176_i32, %0 : i32
        %true_21 = arith.constant true
        %62 = "simt_step.wave_count_bits"(%true_21) : (i1) -> i32
        "simt_step.buffer.store"(%arg0, %61, %62) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
        "simt_step.yield"(%60#0) {fallthrough = false} : (i32) -> ()
      ^bb2(%63: i32):  // no predecessors
        %c52_i32 = arith.constant 52 : i32
        %64 = arith.addi %c52_i32, %0 : i32
        %65 = "simt_step.buffer.load"(%arg1, %64) : (!simt_step.resource<Global, i32>, i32) -> i32
        %66 = simt_step.subgroup_id
        %67 = arith.index_cast %66 : index to i32
        %68 = "simt_step.switch"(%65, %67) ({
        ^bb0(%arg4: i32):
          %75 = simt_step.lane_id
          %76 = arith.index_cast %75 : index to i32
          "simt_step.yield"(%76) {fallthrough = true} : (i32) -> ()
        ^bb1(%77: i32):  // no predecessors
          %78 = simt_step.subgroup_id
          %79 = arith.index_cast %78 : index to i32
          %true_28 = arith.constant true
          %c192_i32 = arith.constant 192 : i32
          %80 = arith.addi %c192_i32, %0 : i32
          %true_29 = arith.constant true
          %81 = "simt_step.wave_count_bits"(%true_29) : (i1) -> i32
          "simt_step.buffer.store"(%arg0, %80, %81) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
          "simt_step.yield"(%79) {fallthrough = false} : (i32) -> ()
        }) {case_values = array<i64: 0>, default_index = 0 : i64} : (i32, i32) -> i32
        %true_22 = arith.constant true
        %c208_i32 = arith.constant 208 : i32
        %69 = arith.addi %c208_i32, %0 : i32
        %true_23 = arith.constant true
        %70 = "simt_step.wave_count_bits"(%true_23) : (i1) -> i32
        "simt_step.buffer.store"(%arg0, %69, %70) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
        %true_24 = arith.constant true
        %c224_i32 = arith.constant 224 : i32
        %71 = arith.addi %c224_i32, %0 : i32
        %true_25 = arith.constant true
        %72 = "simt_step.wave_count_bits"(%true_25) : (i1) -> i32
        "simt_step.buffer.store"(%arg0, %71, %72) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
        "simt_step.yield"(%68) {fallthrough = true} : (i32) -> ()
      ^bb3(%73: i32):  // no predecessors
        %c0_i32_26 = arith.constant 0 : i32
        %c0_i32_27 = arith.constant 0 : i32
        %74:2 = "simt_step.loop"(%c0_i32_26, %c0_i32_27) ({
        ^bb0(%arg4: i32, %arg5: i32):
          %c4_i32_28 = arith.constant 4 : i32
          %75 = arith.muli %arg5, %c4_i32_28 : i32
          %76 = arith.addi %75, %0 : i32
          %c56_i32 = arith.constant 56 : i32
          %77 = arith.addi %c56_i32, %76 : i32
          %78 = "simt_step.buffer.load"(%arg1, %77) : (!simt_step.resource<Global, i32>, i32) -> i32
          %c0_i32_29 = arith.constant 0 : i32
          %79 = arith.cmpi ne, %78, %c0_i32_29 : i32
          "simt_step.condition"(%79, %arg4, %arg5) : (i1, i32, i32) -> ()
        }, {
        ^bb0(%arg4: i32, %arg5: i32):
          %75 = arith.addi %arg4, %arg5 : i32
          %c1_i32 = arith.constant 1 : i32
          %76 = arith.addi %arg5, %c1_i32 : i32
          %true_28 = arith.constant true
          %c240_i32 = arith.constant 240 : i32
          %c4_i32_29 = arith.constant 4 : i32
          %77 = arith.muli %arg5, %c4_i32_29 : i32
          %78 = arith.addi %c240_i32, %77 : i32
          %79 = arith.addi %78, %0 : i32
          %true_30 = arith.constant true
          %80 = "simt_step.wave_count_bits"(%true_30) : (i1) -> i32
          "simt_step.buffer.store"(%arg0, %79, %80) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
          "simt_step.yield"(%75, %76) : (i32, i32) -> ()
        }) : (i32, i32) -> (i32, i32)
        "simt_step.yield"(%74#0) {fallthrough = false} : (i32) -> ()
      }) {case_values = array<i64: 0, 1, 2>, default_index = 0 : i64} : (i32, i32) -> i32
      %true_6 = arith.constant true
      %c256_i32 = arith.constant 256 : i32
      %26 = arith.addi %c256_i32, %0 : i32
      %true_7 = arith.constant true
      %27 = "simt_step.wave_count_bits"(%true_7) : (i1) -> i32
      "simt_step.buffer.store"(%arg0, %26, %27) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
      %true_8 = arith.constant true
      %c272_i32 = arith.constant 272 : i32
      %28 = arith.addi %c272_i32, %0 : i32
      %true_9 = arith.constant true
      %29 = "simt_step.wave_count_bits"(%true_9) : (i1) -> i32
      "simt_step.buffer.store"(%arg0, %28, %29) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
      "simt_step.yield"(%25) {fallthrough = true} : (i32) -> ()
    ^bb1(%30: i32):  // no predecessors
      %c76_i32 = arith.constant 76 : i32
      %31 = arith.addi %c76_i32, %0 : i32
      %32 = "simt_step.buffer.load"(%arg1, %31) : (!simt_step.resource<Global, i32>, i32) -> i32
      %c0_i32_10 = arith.constant 0 : i32
      %33 = arith.cmpi ne, %32, %c0_i32_10 : i32
      %34 = "simt_step.if"(%33) ({
        %c80_i32 = arith.constant 80 : i32
        %54 = arith.addi %c80_i32, %0 : i32
        %55 = "simt_step.buffer.load"(%arg1, %54) : (!simt_step.resource<Global, i32>, i32) -> i32
        %c0_i32_18 = arith.constant 0 : i32
        %56 = arith.cmpi ne, %55, %c0_i32_18 : i32
        %57 = "simt_step.if"(%56) ({
          %c4_i32_20 = arith.constant 4 : i32
          "simt_step.yield"(%c4_i32_20) : (i32) -> ()
        }, {
          %60 = simt_step.subgroup_id
          %61 = arith.index_cast %60 : index to i32
          "simt_step.yield"(%61) : (i32) -> ()
        }) : (i1) -> i32
        %c288_i32 = arith.constant 288 : i32
        %58 = arith.addi %c288_i32, %0 : i32
        %true_19 = arith.constant true
        %59 = "simt_step.wave_count_bits"(%true_19) : (i1) -> i32
        "simt_step.buffer.store"(%arg0, %58, %59) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
        "simt_step.yield"(%57) : (i32) -> ()
      }, {
        %c84_i32 = arith.constant 84 : i32
        %54 = arith.addi %c84_i32, %0 : i32
        %55 = "simt_step.buffer.load"(%arg1, %54) : (!simt_step.resource<Global, i32>, i32) -> i32
        %c3_i32_18 = arith.constant 3 : i32
        %56 = "simt_step.switch"(%55, %c3_i32_18) ({
        ^bb0(%arg3: i32):
          %57 = simt_step.lane_id
          %58 = arith.index_cast %57 : index to i32
          "simt_step.yield"(%58) {fallthrough = true} : (i32) -> ()
        ^bb1(%59: i32):  // no predecessors
          %c0_i32_19 = arith.constant 0 : i32
          "simt_step.yield"(%c0_i32_19) {fallthrough = false} : (i32) -> ()
        ^bb2(%60: i32):  // no predecessors
          %61 = simt_step.subgroup_id
          %62 = arith.index_cast %61 : index to i32
          %true_20 = arith.constant true
          %c304_i32 = arith.constant 304 : i32
          %63 = arith.addi %c304_i32, %0 : i32
          %true_21 = arith.constant true
          %64 = "simt_step.wave_count_bits"(%true_21) : (i1) -> i32
          "simt_step.buffer.store"(%arg0, %63, %64) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
          "simt_step.yield"(%62) {fallthrough = false} : (i32) -> ()
        }) {case_values = array<i64: 0, 1>, default_index = 0 : i64} : (i32, i32) -> i32
        "simt_step.yield"(%56) : (i32) -> ()
      }) : (i1) -> i32
      %c320_i32 = arith.constant 320 : i32
      %35 = arith.addi %c320_i32, %0 : i32
      %true_11 = arith.constant true
      %36 = "simt_step.wave_count_bits"(%true_11) : (i1) -> i32
      "simt_step.buffer.store"(%arg0, %35, %36) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
      %true_12 = arith.constant true
      %c336_i32 = arith.constant 336 : i32
      %37 = arith.addi %c336_i32, %0 : i32
      %true_13 = arith.constant true
      %38 = "simt_step.wave_count_bits"(%true_13) : (i1) -> i32
      "simt_step.buffer.store"(%arg0, %37, %38) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
      "simt_step.yield"(%34) {fallthrough = false} : (i32) -> ()
    ^bb2(%39: i32):  // no predecessors
      %c88_i32 = arith.constant 88 : i32
      %40 = arith.addi %c88_i32, %0 : i32
      %41 = "simt_step.buffer.load"(%arg1, %40) : (!simt_step.resource<Global, i32>, i32) -> i32
      %42 = simt_step.lane_id
      %43 = arith.index_cast %42 : index to i32
      %44 = "simt_step.switch"(%41, %43) ({
      ^bb0(%arg3: i32):
        %c0_i32_18 = arith.constant 0 : i32
        %c0_i32_19 = arith.constant 0 : i32
        %54:2 = "simt_step.loop"(%c0_i32_18, %c0_i32_19) ({
        ^bb0(%arg4: i32, %arg5: i32):
          %c4_i32_26 = arith.constant 4 : i32
          %71 = arith.muli %arg5, %c4_i32_26 : i32
          %72 = arith.addi %71, %0 : i32
          %c92_i32 = arith.constant 92 : i32
          %73 = arith.addi %c92_i32, %72 : i32
          %74 = "simt_step.buffer.load"(%arg1, %73) : (!simt_step.resource<Global, i32>, i32) -> i32
          %c0_i32_27 = arith.constant 0 : i32
          %75 = arith.cmpi ne, %74, %c0_i32_27 : i32
          "simt_step.condition"(%75, %arg4, %arg5) : (i1, i32, i32) -> ()
        }, {
        ^bb0(%arg4: i32, %arg5: i32):
          %71 = arith.addi %arg4, %arg5 : i32
          %c1_i32 = arith.constant 1 : i32
          %72 = arith.addi %arg5, %c1_i32 : i32
          %true_26 = arith.constant true
          %c352_i32 = arith.constant 352 : i32
          %c4_i32_27 = arith.constant 4 : i32
          %73 = arith.muli %arg5, %c4_i32_27 : i32
          %74 = arith.addi %c352_i32, %73 : i32
          %75 = arith.addi %74, %0 : i32
          %true_28 = arith.constant true
          %76 = "simt_step.wave_count_bits"(%true_28) : (i1) -> i32
          "simt_step.buffer.store"(%arg0, %75, %76) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
          "simt_step.yield"(%71, %72) : (i32, i32) -> ()
        }) : (i32, i32) -> (i32, i32)
        %true_20 = arith.constant true
        %c368_i32 = arith.constant 368 : i32
        %55 = arith.addi %c368_i32, %0 : i32
        %true_21 = arith.constant true
        %56 = "simt_step.wave_count_bits"(%true_21) : (i1) -> i32
        "simt_step.buffer.store"(%arg0, %55, %56) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
        "simt_step.yield"(%54#0) {fallthrough = false} : (i32) -> ()
      ^bb1(%57: i32):  // no predecessors
        %c112_i32 = arith.constant 112 : i32
        %58 = arith.addi %c112_i32, %0 : i32
        %59 = "simt_step.buffer.load"(%arg1, %58) : (!simt_step.resource<Global, i32>, i32) -> i32
        %c0_i32_22 = arith.constant 0 : i32
        %60 = arith.cmpi ne, %59, %c0_i32_22 : i32
        %61 = "simt_step.if"(%60) ({
          %c2_i32 = arith.constant 2 : i32
          %71 = arith.addi %0, %c2_i32 : i32
          "simt_step.yield"(%71) : (i32) -> ()
        }, {
          %c0_i32_26 = arith.constant 0 : i32
          "simt_step.yield"(%c0_i32_26) : (i32) -> ()
        }) : (i1) -> i32
        %c384_i32 = arith.constant 384 : i32
        %62 = arith.addi %c384_i32, %0 : i32
        %true_23 = arith.constant true
        %63 = "simt_step.wave_count_bits"(%true_23) : (i1) -> i32
        "simt_step.buffer.store"(%arg0, %62, %63) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
        "simt_step.yield"(%61) {fallthrough = false} : (i32) -> ()
      ^bb2(%64: i32):  // no predecessors
        %c116_i32 = arith.constant 116 : i32
        %65 = arith.addi %c116_i32, %0 : i32
        %66 = "simt_step.buffer.load"(%arg1, %65) : (!simt_step.resource<Global, i32>, i32) -> i32
        %c0_i32_24 = arith.constant 0 : i32
        %67 = arith.cmpi ne, %66, %c0_i32_24 : i32
        %68 = "simt_step.if"(%67) ({
          %71 = simt_step.subgroup_id
          %72 = arith.index_cast %71 : index to i32
          "simt_step.yield"(%72) : (i32) -> ()
        }, {
          "simt_step.yield"(%0) : (i32) -> ()
        }) : (i1) -> i32
        %c400_i32 = arith.constant 400 : i32
        %69 = arith.addi %c400_i32, %0 : i32
        %true_25 = arith.constant true
        %70 = "simt_step.wave_count_bits"(%true_25) : (i1) -> i32
        "simt_step.buffer.store"(%arg0, %69, %70) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
        "simt_step.yield"(%68) {fallthrough = false} : (i32) -> ()
      }) {case_values = array<i64: 0, 1>, default_index = 1 : i64} : (i32, i32) -> i32
      %true_14 = arith.constant true
      %c416_i32 = arith.constant 416 : i32
      %45 = arith.addi %c416_i32, %0 : i32
      %true_15 = arith.constant true
      %46 = "simt_step.wave_count_bits"(%true_15) : (i1) -> i32
      "simt_step.buffer.store"(%arg0, %45, %46) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
      %true_16 = arith.constant true
      %c432_i32 = arith.constant 432 : i32
      %47 = arith.addi %c432_i32, %0 : i32
      %true_17 = arith.constant true
      %48 = "simt_step.wave_count_bits"(%true_17) : (i1) -> i32
      "simt_step.buffer.store"(%arg0, %47, %48) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
      "simt_step.yield"(%44) {fallthrough = false} : (i32) -> ()
    ^bb3(%49: i32):  // no predecessors
      %c120_i32 = arith.constant 120 : i32
      %50 = arith.addi %c120_i32, %0 : i32
      %51 = "simt_step.buffer.load"(%arg1, %50) : (!simt_step.resource<Global, i32>, i32) -> i32
      %c3_i32 = arith.constant 3 : i32
      %52 = arith.addi %0, %c3_i32 : i32
      %53 = "simt_step.switch"(%51, %52) ({
      ^bb0(%arg3: i32):
        %c124_i32 = arith.constant 124 : i32
        %54 = arith.addi %c124_i32, %0 : i32
        %55 = "simt_step.buffer.load"(%arg1, %54) : (!simt_step.resource<Global, i32>, i32) -> i32
        %c4_i32_18 = arith.constant 4 : i32
        %56 = "simt_step.switch"(%55, %c4_i32_18) ({
        ^bb0(%arg4: i32):
          %c4_i32_24 = arith.constant 4 : i32
          %66 = arith.addi %0, %c4_i32_24 : i32
          %true_25 = arith.constant true
          %c448_i32 = arith.constant 448 : i32
          %67 = arith.addi %c448_i32, %0 : i32
          %true_26 = arith.constant true
          %68 = "simt_step.wave_count_bits"(%true_26) : (i1) -> i32
          "simt_step.buffer.store"(%arg0, %67, %68) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
          "simt_step.yield"(%66) {fallthrough = false} : (i32) -> ()
        ^bb1(%69: i32):  // no predecessors
          "simt_step.yield"(%0) {fallthrough = false} : (i32) -> ()
        }) {case_values = array<i64: 0>, default_index = 1 : i64} : (i32, i32) -> i32
        %true_19 = arith.constant true
        %c464_i32 = arith.constant 464 : i32
        %57 = arith.addi %c464_i32, %0 : i32
        %true_20 = arith.constant true
        %58 = "simt_step.wave_count_bits"(%true_20) : (i1) -> i32
        "simt_step.buffer.store"(%arg0, %57, %58) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
        "simt_step.yield"(%56) {fallthrough = false} : (i32) -> ()
      ^bb1(%59: i32):  // no predecessors
        %c4_i32_21 = arith.constant 4 : i32
        %60 = arith.addi %0, %c4_i32_21 : i32
        "simt_step.yield"(%60) {fallthrough = true} : (i32) -> ()
      ^bb2(%61: i32):  // no predecessors
        %c128_i32_22 = arith.constant 128 : i32
        %62 = arith.addi %c128_i32_22, %0 : i32
        %63 = "simt_step.buffer.load"(%arg1, %62) : (!simt_step.resource<Global, i32>, i32) -> i32
        %c3_i32_23 = arith.constant 3 : i32
        %64 = arith.addi %0, %c3_i32_23 : i32
        %65 = "simt_step.switch"(%63, %64) ({
        ^bb0(%arg4: i32):
          %66 = simt_step.subgroup_id
          %67 = arith.index_cast %66 : index to i32
          %true_24 = arith.constant true
          %c480_i32 = arith.constant 480 : i32
          %68 = arith.addi %c480_i32, %0 : i32
          %true_25 = arith.constant true
          %69 = "simt_step.wave_count_bits"(%true_25) : (i1) -> i32
          "simt_step.buffer.store"(%arg0, %68, %69) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
          "simt_step.yield"(%67) {fallthrough = true} : (i32) -> ()
        ^bb1(%70: i32):  // no predecessors
          "simt_step.yield"(%0) {fallthrough = false} : (i32) -> ()
        }) {case_values = array<i64: 0>, default_index = 1 : i64} : (i32, i32) -> i32
        "simt_step.yield"(%65) {fallthrough = false} : (i32) -> ()
      }) {case_values = array<i64: 0, 1>, default_index = 1 : i64} : (i32, i32) -> i32
      "simt_step.yield"(%53) {fallthrough = false} : (i32) -> ()
    }) {case_values = array<i64: 0, 1, 2>, default_index = 1 : i64} : (i32, i32) -> i32
    %true_2 = arith.constant true
    %c496_i32 = arith.constant 496 : i32
    %17 = arith.addi %c496_i32, %0 : i32
    %true_3 = arith.constant true
    %18 = "simt_step.wave_count_bits"(%true_3) : (i1) -> i32
    "simt_step.buffer.store"(%arg0, %17, %18) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
    %c132_i32 = arith.constant 132 : i32
    %19 = arith.addi %c132_i32, %0 : i32
    %20 = "simt_step.buffer.load"(%arg1, %19) : (!simt_step.resource<Global, i32>, i32) -> i32
    %c4_i32_4 = arith.constant 4 : i32
    %21 = arith.addi %0, %c4_i32_4 : i32
    %22 = "simt_step.switch"(%20, %21) ({
    ^bb0(%arg2: i32):
      %c136_i32 = arith.constant 136 : i32
      %23 = arith.addi %c136_i32, %0 : i32
      %24 = "simt_step.buffer.load"(%arg1, %23) : (!simt_step.resource<Global, i32>, i32) -> i32
      %c0_i32_5 = arith.constant 0 : i32
      %25 = arith.cmpi ne, %24, %c0_i32_5 : i32
      %26 = "simt_step.if"(%25) ({
        %c0_i32_12 = arith.constant 0 : i32
        %c0_i32_13 = arith.constant 0 : i32
        %42:2 = "simt_step.loop"(%c0_i32_12, %c0_i32_13) ({
        ^bb0(%arg3: i32, %arg4: i32):
          %c4_i32_14 = arith.constant 4 : i32
          %43 = arith.muli %arg4, %c4_i32_14 : i32
          %44 = arith.addi %43, %0 : i32
          %c140_i32 = arith.constant 140 : i32
          %45 = arith.addi %c140_i32, %44 : i32
          %46 = "simt_step.buffer.load"(%arg1, %45) : (!simt_step.resource<Global, i32>, i32) -> i32
          %c0_i32_15 = arith.constant 0 : i32
          %47 = arith.cmpi ne, %46, %c0_i32_15 : i32
          "simt_step.condition"(%47, %arg3, %arg4) : (i1, i32, i32) -> ()
        }, {
        ^bb0(%arg3: i32, %arg4: i32):
          %43 = arith.addi %arg3, %arg4 : i32
          %c1_i32 = arith.constant 1 : i32
          %44 = arith.addi %arg4, %c1_i32 : i32
          %true_14 = arith.constant true
          %c512_i32 = arith.constant 512 : i32
          %c4_i32_15 = arith.constant 4 : i32
          %45 = arith.muli %arg4, %c4_i32_15 : i32
          %46 = arith.addi %c512_i32, %45 : i32
          %47 = arith.addi %46, %0 : i32
          %true_16 = arith.constant true
          %48 = "simt_step.wave_count_bits"(%true_16) : (i1) -> i32
          "simt_step.buffer.store"(%arg0, %47, %48) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
          "simt_step.yield"(%43, %44) : (i32, i32) -> ()
        }) : (i32, i32) -> (i32, i32)
        "simt_step.yield"(%42#0) : (i32) -> ()
      }, {
        %c4_i32_12 = arith.constant 4 : i32
        %42 = arith.addi %0, %c4_i32_12 : i32
        "simt_step.yield"(%42) : (i32) -> ()
      }) : (i1) -> i32
      %c528_i32 = arith.constant 528 : i32
      %27 = arith.addi %c528_i32, %0 : i32
      %true_6 = arith.constant true
      %28 = "simt_step.wave_count_bits"(%true_6) : (i1) -> i32
      "simt_step.buffer.store"(%arg0, %27, %28) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
      %true_7 = arith.constant true
      %c544_i32 = arith.constant 544 : i32
      %29 = arith.addi %c544_i32, %0 : i32
      %true_8 = arith.constant true
      %30 = "simt_step.wave_count_bits"(%true_8) : (i1) -> i32
      "simt_step.buffer.store"(%arg0, %29, %30) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
      "simt_step.yield"(%26) {fallthrough = false} : (i32) -> ()
    ^bb1(%31: i32):  // no predecessors
      %c160_i32 = arith.constant 160 : i32
      %32 = arith.addi %c160_i32, %0 : i32
      %33 = "simt_step.buffer.load"(%arg1, %32) : (!simt_step.resource<Global, i32>, i32) -> i32
      %c0_i32_9 = arith.constant 0 : i32
      %34 = "simt_step.switch"(%33, %c0_i32_9) ({
      ^bb0(%arg3: i32):
        %42 = simt_step.subgroup_id
        %43 = arith.index_cast %42 : index to i32
        %true_12 = arith.constant true
        %c560_i32 = arith.constant 560 : i32
        %44 = arith.addi %c560_i32, %0 : i32
        %true_13 = arith.constant true
        %45 = "simt_step.wave_count_bits"(%true_13) : (i1) -> i32
        "simt_step.buffer.store"(%arg0, %44, %45) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
        "simt_step.yield"(%43) {fallthrough = true} : (i32) -> ()
      ^bb1(%46: i32):  // no predecessors
        %c0_i32_14 = arith.constant 0 : i32
        %c0_i32_15 = arith.constant 0 : i32
        %47:2 = "simt_step.loop"(%c0_i32_14, %c0_i32_15) ({
        ^bb0(%arg4: i32, %arg5: i32):
          %c4_i32_16 = arith.constant 4 : i32
          %48 = arith.muli %arg5, %c4_i32_16 : i32
          %49 = arith.addi %48, %0 : i32
          %c164_i32 = arith.constant 164 : i32
          %50 = arith.addi %c164_i32, %49 : i32
          %51 = "simt_step.buffer.load"(%arg1, %50) : (!simt_step.resource<Global, i32>, i32) -> i32
          %c0_i32_17 = arith.constant 0 : i32
          %52 = arith.cmpi ne, %51, %c0_i32_17 : i32
          "simt_step.condition"(%52, %arg4, %arg5) : (i1, i32, i32) -> ()
        }, {
        ^bb0(%arg4: i32, %arg5: i32):
          %48 = arith.addi %arg4, %arg5 : i32
          %c1_i32 = arith.constant 1 : i32
          %49 = arith.addi %arg5, %c1_i32 : i32
          %true_16 = arith.constant true
          %c576_i32 = arith.constant 576 : i32
          %c4_i32_17 = arith.constant 4 : i32
          %50 = arith.muli %arg5, %c4_i32_17 : i32
          %51 = arith.addi %c576_i32, %50 : i32
          %52 = arith.addi %51, %0 : i32
          %true_18 = arith.constant true
          %53 = "simt_step.wave_count_bits"(%true_18) : (i1) -> i32
          "simt_step.buffer.store"(%arg0, %52, %53) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
          "simt_step.yield"(%48, %49) : (i32, i32) -> ()
        }) : (i32, i32) -> (i32, i32)
        "simt_step.yield"(%47#0) {fallthrough = false} : (i32) -> ()
      }) {case_values = array<i64: 0>, default_index = 1 : i64} : (i32, i32) -> i32
      "simt_step.yield"(%34) {fallthrough = false} : (i32) -> ()
    ^bb2(%35: i32):  // no predecessors
      %c184_i32 = arith.constant 184 : i32
      %36 = arith.addi %c184_i32, %0 : i32
      %37 = "simt_step.buffer.load"(%arg1, %36) : (!simt_step.resource<Global, i32>, i32) -> i32
      %c0_i32_10 = arith.constant 0 : i32
      %38 = arith.cmpi ne, %37, %c0_i32_10 : i32
      %39 = "simt_step.if"(%38) ({
        %c0_i32_12 = arith.constant 0 : i32
        %c0_i32_13 = arith.constant 0 : i32
        %42:2 = "simt_step.loop"(%c0_i32_12, %c0_i32_13) ({
        ^bb0(%arg3: i32, %arg4: i32):
          %c4_i32_14 = arith.constant 4 : i32
          %43 = arith.muli %arg4, %c4_i32_14 : i32
          %44 = arith.addi %43, %0 : i32
          %c188_i32 = arith.constant 188 : i32
          %45 = arith.addi %c188_i32, %44 : i32
          %46 = "simt_step.buffer.load"(%arg1, %45) : (!simt_step.resource<Global, i32>, i32) -> i32
          %c0_i32_15 = arith.constant 0 : i32
          %47 = arith.cmpi ne, %46, %c0_i32_15 : i32
          "simt_step.condition"(%47, %arg3, %arg4) : (i1, i32, i32) -> ()
        }, {
        ^bb0(%arg3: i32, %arg4: i32):
          %43 = arith.addi %arg3, %arg4 : i32
          %c1_i32 = arith.constant 1 : i32
          %44 = arith.addi %arg4, %c1_i32 : i32
          %true_14 = arith.constant true
          %c592_i32 = arith.constant 592 : i32
          %c4_i32_15 = arith.constant 4 : i32
          %45 = arith.muli %arg4, %c4_i32_15 : i32
          %46 = arith.addi %c592_i32, %45 : i32
          %47 = arith.addi %46, %0 : i32
          %true_16 = arith.constant true
          %48 = "simt_step.wave_count_bits"(%true_16) : (i1) -> i32
          "simt_step.buffer.store"(%arg0, %47, %48) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
          "simt_step.yield"(%43, %44) : (i32, i32) -> ()
        }) : (i32, i32) -> (i32, i32)
        "simt_step.yield"(%42#0) : (i32) -> ()
      }, {
        %c208_i32 = arith.constant 208 : i32
        %42 = arith.addi %c208_i32, %0 : i32
        %43 = "simt_step.buffer.load"(%arg1, %42) : (!simt_step.resource<Global, i32>, i32) -> i32
        %c4_i32_12 = arith.constant 4 : i32
        %44 = "simt_step.switch"(%43, %c4_i32_12) ({
        ^bb0(%arg3: i32):
          %c4_i32_13 = arith.constant 4 : i32
          "simt_step.yield"(%c4_i32_13) {fallthrough = false} : (i32) -> ()
        ^bb1(%45: i32):  // no predecessors
          %true_14 = arith.constant true
          %c608_i32 = arith.constant 608 : i32
          %46 = arith.addi %c608_i32, %0 : i32
          %true_15 = arith.constant true
          %47 = "simt_step.wave_count_bits"(%true_15) : (i1) -> i32
          "simt_step.buffer.store"(%arg0, %46, %47) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
          "simt_step.yield"(%0) {fallthrough = false} : (i32) -> ()
        }) {case_values = array<i64: 0>, default_index = 1 : i64} : (i32, i32) -> i32
        "simt_step.yield"(%44) : (i32) -> ()
      }) : (i1) -> i32
      %c624_i32 = arith.constant 624 : i32
      %40 = arith.addi %c624_i32, %0 : i32
      %true_11 = arith.constant true
      %41 = "simt_step.wave_count_bits"(%true_11) : (i1) -> i32
      "simt_step.buffer.store"(%arg0, %40, %41) : (!simt_step.resource<Global, i32>, i32, i32) -> ()
      "simt_step.yield"(%39) {fallthrough = false} : (i32) -> ()
    }) {case_values = array<i64: 0, 1>, default_index = 0 : i64} : (i32, i32) -> i32
    return
  }
}

